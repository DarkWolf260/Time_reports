...
Sigue el patrón de diseño de la aplicación, separando UI, estado y servicios.
"""
import flet as ft
import datetime
...
# Importación del servicio de alarmas que se creará más adelante
...
class AlarmsTab(ft.Column):
    ...
    def __init__(self, app_state: AppState):
        ...
        self.alarm_service = AlarmService() # Se instanciará cuando esté implementado
    ...
    def _create_components(self):
        ...
        self.alarm_type_radio = ft.RadioGroup(
            content=ft.Row([
        ...
        self.add_alarm_button = ft.ElevatedButton(
            text="Añadir Alarma",
            on_click=self._add_alarm_clicked,
            style=ButtonStyles.primary()
        )

        self.alarms_list_view = ft.ListView(
            spacing=10,
            padding=10,
            auto_scroll=True
        )
    ...
    def _build_layout(self):
        ...
        time_selection_row = ft.Row(
            [ft.Text("Hora:", size=16), self.selected_time_text, self.pick_time_button],
    ...
    def will_unmount(self):
        """Se llama cuando el control se elimina de la página."""
        self.running = False
    ...
    def _add_alarm_clicked(self, e):
        """Maneja el clic en el botón de añadir alarma."""
        ...
        # Lógica de negocio movida a AlarmManager
        new_alarm = self.app_state.alarm_manager.agregar_alarma(
            self.selected_alarm_time, alarm_type
        )

        if new_alarm:
            # Lógica de programación de la alarma nativa (se descomentará más tarde)
            self.alarm_service.schedule_alarm(new_alarm)
    ...
    def _delete_alarm_clicked(self, alarm_id: str):
        """Maneja la eliminación de una alarma."""
        alarm = self.app_state.alarm_manager.buscar_por_id(alarm_id)
        if alarm:
            # Cancelar la alarma nativa (se descomentará más tarde)
            self.alarm_service.cancel_alarm(alarm)

            # Eliminar del estado
            self.app_state.alarm_manager.eliminar_alarma(alarm_id)

            self._show_snackbar(f"Alarma de las {alarm.time} eliminada.")
            self._update_alarms_list()

        self.page.update()

    def _update_alarms_list(self):
        """Actualiza la lista de alarmas en la UI."""
        ...
        if not alarms:
            self.alarms_list_view.controls.append(
                ft.Text("No hay alarmas programadas.", text_align=ft.TextAlign.CENTER, italic=True)
            )
        else:
            for alarm in sorted(alarms, key=lambda x: x.time):
                self.alarms_list_view.controls.append(
                    AlarmCard(alarm=alarm, on_delete=self._delete_alarm_clicked)
                )
        self.update()
    ...
    def update_theme(self):
        """Actualiza el tema de la pestaña y sus componentes."""
